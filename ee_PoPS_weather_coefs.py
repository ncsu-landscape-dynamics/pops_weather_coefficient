...
if prcp_method == "polynomial":
    ...
    raise ValueError("not a recognized prcp_method")
...
if rh_method == "polynomial":
    ...
    raise ValueError("not a recognized rh_method")
...
if temp_method == "reclass":
    ...
    raise ValueError("not a recognized temp_method")
...
if data_selector == "prism":
    ...
    raise ValueError("not a recognized data source.")
...
for y in year_list:
    ...
    for i in range(len(date_list) - 1):
        ...
        # NOTE: It's not clear to me if we need these if statements since GEE relies
        # so much on delayed execution. This may not cost any execution time because
        # it may not run until the call to export. Should check on that.
        if temperature:
    ...
    if temperature:
        tasks = ee.batch.Export.image.toDrive(
            ...
            scale=output_scale,
            # crs=str(img_crs),
            # crsTransform=img_transform.to_gdal(),
        )
        tasks.start()
...
# with tqdm(total=len(task_status_list), desc="GEE processing") as pbar:
